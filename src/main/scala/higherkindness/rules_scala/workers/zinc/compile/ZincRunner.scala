...
import xsbti.compile.{AnalysisContents, ClasspathOptionsUtil, CompileAnalysis, CompileOptions, CompileProgress, CompilerCache, DefinesClass, DeleteImmediatelyManagerType, IncOptions, Inputs, MiniSetup, PerClasspathEntryLookup, PreviousResult, Setup, TastyFiles}
  ...
  protected[this] def work(worker: Namespace, args: Array[String], out: PrintStream) = {
    ...
    val setup = {
      val incOptions = IncOptions.create()
        ...
        .withClassfileManagerType(DeleteImmediatelyManagerType.of())
